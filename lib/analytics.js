...
/** Digits-only UK-normalised phone, for comparing two spellings of one number. */
...
/**
 * Whole calendar days between two "YYYY-MM-DD" keys. Parsed as UTC so DST
 * changes can't turn a day into 23 or 25 hours and skew the count.
 */
...
/** "DD/MM" part only — older records have no year, so the year can't be compared. */
function birthdayDayMonth(value) {
  const m = String(value || "").match(/^(\d{2})\/(\d{2})/);
  return m ? `${m[1]}/${m[2]}` : "";
}

/**
 * Collapse visits into ONE record per person.
 *
 * Identity is `guestKey` — the email, or the device if there isn't one. Guests
 * retype the form on every visit, so contact details are taken from their most
 * recent visit that actually filled the field in: a blank phone on the latest
 * visit must not erase a good number from an earlier one.
 *
 * Where a field genuinely changed between visits (a mistyped phone, a birthday
 * entered as 7 Aug once and 7 Sep twice) every variant is kept in `conflicts`
 * so the dashboard can flag it rather than silently pick one.
 *
 * Pass the FULL dataset — lifetime figures are the point. Scope to a date
 * window afterwards, by checking which keys appear in the filtered visits.
 */
export function customerRoster(entries, now = new Date()) {
  const map = new Map();
  const FIELDS = [
    ["name", (e) => e.firstName, (v) => String(v).trim().toLowerCase()],
    ["phone", (e) => e.phone, normalisePhone],
    ["birthday", (e) => e.birthday, birthdayDayMonth],
    ["email", (e) => e.email, (v) => String(v).trim().toLowerCase()],
  ];
  ...
  for (const e of entries) {
    ...
    // Parse the date before touching the map — otherwise an unreadable
    // timestamp leaves behind a customer with zero visits and no dates.
    ...
    if (!c) {
      c = {
        ...
        visits: 0, first: null, last: null,
        minutes: 0, staysWithTime: 0,
        branches: new Set(), optedIn: false,
        conflicts: {},
        _at: {},      // when each field's current value was recorded
        _seen: {},    // normalised value -> raw value, per field
      };
      map.set(key, c);
    }
    ...
    if (e.totalMinutes > 0) {
      c.minutes += e.totalMinutes;
      c.staysWithTime += 1;
    }
    // "Unknown" matches the label used by the sidebar and the visit table, so
    // clicking that store still finds the people who came through it.
    c.branches.add(e.branch || "Unknown");
    if (String(e.promo || "").toLowerCase() === "yes") c.optedIn = true;

    for (const [field, get, norm] of FIELDS) {
      const raw = get(e);
      if (!raw || !String(raw).trim()) continue;   // never overwrite with a blank
      const n = norm(raw);
      if (!n) continue;
      if (!c._seen[field]) c._seen[field] = new Map();
      if (!c._seen[field].has(n)) c._seen[field].set(n, String(raw).trim());
      // Most recent non-empty value wins.
      if (c._at[field] === undefined || t >= c._at[field]) {
        c[field] = String(raw).trim();
        c._at[field] = t;
      }
    }
  }

  // Calendar days apart, measured in the business timezone — otherwise a
  // dashboard opened outside the UK labels visits "Yesterday" that the date
  // filter counted as today. Note: zonedParts, not toDateKey — toDateKey
  // deliberately reads a Date as machine-local for the date pickers.
  const nowParts = zonedParts(now);
  const todayKey = nowParts ? nowParts.dateKey : "";
  const out = [];
  for (const c of map.values()) {
    for (const field of Object.keys(c._seen)) {
      const variants = Array.from(c._seen[field].values());
      if (variants.length > 1) c.conflicts[field] = variants;
    }
    delete c._at;
    delete c._seen;

    c.firstSeen = c.first ? new Date(c.first).toISOString() : "";
    c.lastSeen = c.last ? new Date(c.last).toISOString() : "";
    c.daysSince = daysBetweenKeys(toDateKey(c.lastSeen), todayKey);
    c.avgMinutes = c.staysWithTime ? Math.round(c.minutes / c.staysWithTime) : 0;
    c.tier = loyaltyTier(c.visits);
    c.birthdayIn = daysUntilBirthday(c.birthday, now);
    c.storeList = Array.from(c.branches).sort();
    c.hasConflict = Object.keys(c.conflicts).length > 0;
    out.push(c);
  }

  // Most recent visitors first — the list you'd work down.
  out.sort((a, b) => (b.last || 0) - (a.last || 0));
  return out;
}

/** CSV of customers (one row per person), ready for an email platform. */
export function customersToCsv(customers) {
  const headers = [
    "Name", "Email", "Phone", "Birthday", "Customer Type", "Total Visits",
    "Marketing Opt-in", "First Seen", "Last Seen", "Days Since",
    "Average Stay", "Stores", "Data Conflicts",
  ];
  const esc = (v) => {
    const s = String(v ?? "");
    return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
  ...
  for (const c of customers) {
    lines.push(
      [
        ...
        c.tier.label, c.visits,
        c.optedIn ? "Yes" : "No",
        c.firstSeen ? formatDateTime(c.firstSeen) : "",
        c.lastSeen ? formatDateTime(c.lastSeen) : "",
        c.daysSince === null ? "" : c.daysSince,
        c.avgMinutes ? minutesToLabel(c.avgMinutes) : "",
        c.storeList.join(" / "),
        Object.keys(c.conflicts).join(" "),
      ]
        .map(esc)
        .join(",")
    );
