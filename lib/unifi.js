...
function parseConsoleEntry(entry) {
 ...
 *
 * Three supported forms, combined (later ones don't override earlier):
 *
 *  1. One store per line — preferred, easiest to maintain:
 *       UNIFI_CONSOLE_1=abc:123|BnS Perry Barr
 *       UNIFI_CONSOLE_2=def:456|BnS Castle Vale
 *     Any number is accepted, and gaps in the numbering are fine.
 *
 *  2. Comma-separated list:
 *       UNIFI_CONSOLES=abc:123|BnS Perry Barr,def:456|BnS Castle Vale
 *
 *  3. Legacy single console:
 *       UNIFI_CONSOLE_ID=abc:123
 *
 * Adding a store is therefore a .env change only — never a code change.
 */
function consoles() {
  const found = [];
  const seen = new Set();
  ...
  const add = (entry) => {
    const parsed = parseConsoleEntry(entry);
  ...
  // 1. UNIFI_CONSOLE_<n> — scan every matching env var, sorted numerically.
  ...
  // 2. UNIFI_CONSOLES — comma-separated (newlines tolerated).
  const multi = (process.env.UNIFI_CONSOLES || "").trim();
  if (multi) multi.split(",").forEach(add);

  // 3. Legacy single-console form.
...
module.exports = { authorizeGuest, getConnectedMacs, getConsoles, isGuestAuthorized };
