 ...
 *
 * `deep` controls the fallback scan. The server-side filter is one request;
 * the fallback lists up to 200 clients, so a miss costs two. When racing
 * every console that doubled the request volume for no benefit — the filter
 * either finds the device or it isn't there — so the race passes deep=false
 * and only a targeted single-console check pays for the scan.
 */
async function findClientByMac(consoleId, siteId, mac, { deep = true } = {}) {
  ...
  // The filter answered "no such client". Only rescan when we're allowed to,
  // or when the filter itself broke and we have no answer at all.
 ...
 */
async function probeConsole(con, mac, { deep = true } = {}) {
  const siteId = await getSiteId(con.id);
  const client = await findClientByMac(con.id, siteId, mac, { deep });
...
function raceConsoles(list, mac, batchSize = 6) {
  // deep=false: during a race the fallback 200-client scan would double the
  // request volume without improving the answer.
  return firstSuccess(list, (con) => probeConsole(con, mac, { deep: false }), batchSize);
}
...
// With the lookup now fast, retry sooner and more often instead of waiting
// 1.5s between slow full sweeps.
const FIND_ATTEMPTS = 6;
const FIND_RETRY_MS = 500;
// Hard ceiling on the whole lookup. Per-request timeouts alone don't bound
// this: six attempts against a dead console at 20s each would hold the guest
// on "Connecting…" for two minutes. Better to fail in a few seconds and let
// them retry — their details are already saved either way.
const FIND_BUDGET_MS = 9000;
...
async function authorizeGuest(mac, { apMac = "" } = {}) {
  ...
  for (let attempt = 0; attempt < FIND_ATTEMPTS; attempt++) {
    ...
    try {
      found = useHint
        ? await probeConsole(hinted, mac)
        : await raceConsoles(list, mac);
    } catch (err) {
      lastError = err;
...
async function isGuestAuthorized(mac, consoleId = "") {
  ...
  if (hinted) {
    try {
      const siteId = await getSiteId(hinted.id);
  ...
  for (const con of list) {
    if (hinted && con.id === hinted.id) continue; // already tried
    try {
      const siteId = await getSiteId(con.id);
